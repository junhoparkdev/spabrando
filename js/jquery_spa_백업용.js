(function($, window, document){
    ...
    const spa={
        ...
        header(){
            ...
            const $mobileBtn=$('#header .mobile-btn');

            $(window).scroll(function(e){//스크롤 이벤트 발생하는 동안
                newScroll=$(window).scrollTop();

                if(newScroll-oldScroll > Math.abs(0)){
                    $('#header, .mobile-nav-menu').addClass('pal-on');
                    $('#header, .mobile-nav-menu').removeClass('pal-off');
                }
                if(newScroll===0){
                    $('#header, .mobile-nav-menu').removeClass('pal-on');
                    $('#header, .mobile-nav-menu').addClass('pal-off');
                }
            ...
            $mobileBtn.on({
                click(){
                    $('.mobile-nav-menu').toggleClass('on');
                }
            }); 
            let headerWidth=$('#header').innerHeight();
            $('.mobile-nav-menu').css({top: headerWidth});
            ...
            menuBtn.each(
                function(idx){
                    ...
                    $(this).click(
                        ...
                        caseAddIn.stop().fadeOut(0);
                        caseAddIn.eq(idx).stop().fadeIn(1000);
                    })
                }
            )
            // )
            // function fadeFn(){
            //         caseAddIn.eq(btnNumber).fadeIn(1000)
            //     }
        },
        ...
        section6(){
            ...
            let countNum=[7655, 65444, 5658, 544];
